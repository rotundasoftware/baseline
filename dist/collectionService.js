...
var CollectionService = module.exports = BaseService.extend({
	...
	get: function get(recordId, fieldName) {
	...
	gets: function gets(recordId, fields) {
		var _this = this;
	...
	rawSet: function rawSet(recordId, fieldName, fieldValue) {
		...
		if (_.isUndefined(fieldValue)) delete this._recordsById[recordId][fieldName];else this._recordsById[recordId][fieldName] = this._copyFieldValue(fieldValue);
	...
	destroy: function destroy(recordIdOrIds, options) {
		var _this = this;
		...
		var deleteLocally = function deleteLocally() {
		...
		if (recordIdsToDeleteRemotely.length > 0) {
			...
			deleteLocally();
			return _Promise.resolve({ success: true });
		}
	},
	...
	_copyRecord: function _copyRecord(record) {
		return JSON.parse(JSON.stringify(record));
	},

	_copyFieldValue: function _copyFieldValue(fieldValue) {
		if (fieldValue instanceof Object) return JSON.parse(JSON.stringify(fieldValue));else return fieldValue;
	},
	...
	_mergeDTO: function _mergeDTO(dto, method) {
	...
	_sync: function _sync(url, method, payload, ajaxOptions) {
...
var underscoreTableMethodNames = ['forEach', 'each', 'map', 'collect', 'reduce', 'foldl', 'inject', 'reduceRight', 'foldr', 'find', 'detect', 'filter', 'select', 'reject', 'every', 'all', 'some', 'any', 'include', 'contains', 'invoke', 'max', 'min', 'sortedIndex', 'size', 'first', 'head', 'take', 'initial', 'rest', 'tail', 'drop', 'last', 'without', 'indexOf', 'shuffle', 'lastIndexOf', 'isEmpty'];
