...
var CollectionService = module.exports = BaseService.extend({
  ...
  rawSet: function rawSet(recordId, fieldName, fieldValue) {
    ...
    if (_.isUndefined(fieldValue)) delete this._recordsById[recordId][fieldName];else this._recordsById[recordId][fieldName] = this._cloneFieldValue(fieldValue);
  ...
  _sync: function _sync(url, verb, payload, ajaxOptions) {
    var options = _.defaults({}, options); // Default JSON-request options.
