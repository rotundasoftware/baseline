...
var CollectionService = module.exports = _baseService["default"].extend({
  initialize: function initialize(options) {
    ...
    _backboneEventsStandalone["default"].mixin(this);
  ...
  create: function create(initialFieldValues, options) {
    ...
    if (!newRecordId) initialFieldValues[this._idFieldName] = newRecordId = this._getUniqueId();
  ...
  gets: function gets(recordId, fields, options) {
    ...
    if (!this._recordsById[recordId]) throw new Error('Record id ' + recordId + ' is not present in table \'' + this.collectionName + '\'.');
    if (_underscore["default"].isUndefined(fields)) fields = _underscore["default"].keys(this._recordsById[recordId]);
    if (!_underscore["default"].isArray(fields)) fields = Array.prototype.slice.apply(arguments, [1]);
    var values = {};

    _underscore["default"].each(fields, function (thisFieldName) {
  ...
  sets: function sets(recordId, fields) {
    _underscore["default"].each(fields, function (thisFieldValue, thisFieldName) {
  ...
  destroy: function destroy(recordId, options) {
    ...
    return _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
      var _this, deleteLocally, url, result;
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              deleteLocally = function deleteLocally() {
                ...
                _this._recordIds = _underscore["default"].without(_this._recordIds, recordId);
              ...
              _context.next = 7;
              return _this._sync(url, 'DELETE', undefined, options.ajax);
  ...
  isNew: function isNew(recordId) {
    return _underscore["default"].contains(this._newRecordIds, recordId);
  },
  ...
  merge: function merge(newRecordDTOs) {
    ...
    if (!_underscore["default"].isArray(newRecordDTOs)) newRecordDTOs = [newRecordDTOs];

    _underscore["default"].each(newRecordDTOs, function (thisDto) {
  ...
  toJSON: function toJSON(options) {
    ...
    if (options.recordIds === this._recordIds && !options.clone && !options.fields) {
      ...
      return _underscore["default"].values(this._recordsById);
    } else if (options.fields) {
      return _underscore["default"].map(options.recordIds, function (thisRecordId) {
        if (!_this.isPresent(thisRecordId)) throw new Error('Record id ' + thisRecordId + ' is not present.');
        return this._cloneRecord(_underscore["default"].pick(this._recordsById[thisRecordId], options.fields));
      }, this);
    } else {
      return _underscore["default"].map(options.recordIds, function (thisRecordId) {
        if (!_this.isPresent(thisRecordId)) throw new Error('Record id ' + thisRecordId + ' is not present.');
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              url = _this3._getRESTEndpoint('get', recordId, options.variablePartsOfEndpoint);
              _context2.next = 5;
              return _this._sync(url, 'GET', null, options.ajax);
  ...
  save: function save(recordId, options) {
    ...
    return _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3() {
      var _this, method, isUpdate, url, dto, result;
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              ...
              method = _this4.isNew(recordId) ? 'create' : 'update';
              isUpdate = !_this4.isNew(recordId);
              url = _this._getRESTEndpoint(method, recordId);
              dto = _this4._recordToDTO(recordId, method);
              _context3.next = 8;
              return _this._sync(url, isUpdate ? 'PUT' : 'POST', dto, options.ajax);

            case 8:
              result = _context3.sent;

              if (result.success) {
                if (!isUpdate) _this._newRecordIds = _underscore["default"].without(_this._newRecordIds, recordId);
  ...
  where: function where(attrs, options) {
    ...
    if (_underscore["default"].isEmpty(attrs)) return options.first ? void 0 : [];
    return this[options.first ? 'find' : 'filter'](function (thisRecordId) {
      if (!options.ignoreMissingData) {
        for (var key in attrs) {
          if (_underscore["default"].isUndefined(_this._recordsById[thisRecordId][key])) {
            throw new Error('Field \'' + key + '\' is not present for record id ' + thisRecordId + ' in table \'' + _this.collectionName + '\'.');
          }
        }
      }

      return (0, _matchesWhereQuery["default"])(_this._recordsById[thisRecordId], attrs);
    });
  },
  ...
  registerTapeOperation: function registerTapeOperation(operationName, tapeOperationDescriptor) {
  ...
  _getRESTEndpoint: function _getRESTEndpoint(method, recordIdOrIds, variableParts) {
    ...
    _underscore["default"].each(variableParts, function (thisVariablePartValue, thisVariablePartKey) {
    ...
    if (_underscore["default"].contains(['update', 'delete', 'patch', 'get'], method) && !_underscore["default"].isArray(recordIdOrIds)) {
...
_underscore["default"].each(underscoreTableMethodNames, function (thisMethodName) {
...
_underscore["default"].each(attributeMethods, function (method) {
  CollectionService.prototype[method] = function (value, context) {
    ...
    return _underscore["default"][method](this._recordIds, iterator, context);
  };
});