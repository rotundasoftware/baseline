...
var CollectionService = module.exports = _baseService["default"].extend({
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              url = _this._getRESTEndpoint('delete', recordId);
              _context.next = 6;
  ...
  toJSON: function toJSON(options) {
    ...
    if (options.recordIds === this._recordIds && !options.clone && !options.fields) {
      ...
      return options.recordIds.map(function (thisRecordId) {
        if (!_this2.isPresent(thisRecordId)) throw new Error('Record id ' + thisRecordId + ' is not present.');
        return _this2._cloneRecord(_underscore["default"].pick(_this2._recordsById[thisRecordId], options.fields));
      });
    } else {
      return options.recordIds.map(function (thisRecordId) {
        if (!_this2.isPresent(thisRecordId)) throw new Error('Record id ' + thisRecordId + ' is not present.');
        return _this2._cloneRecord(_this2._recordsById[thisRecordId]);
      });
    }
  },
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              _context2.next = 4;
              return _this3._sync(url, 'GET', null, options.ajax);

            case 4:
              result = _context2.sent;
              if (result.success) _this3._mergeDTO(result.data);
  ...
  _fillInVariablePartsOfRESTEndpoint: function _fillInVariablePartsOfRESTEndpoint(recordId, endpoint) {
    ...
    return endpoint.replace(/\/:(\w+)/g, function (match, fieldName) {
      return '/' + _this6.get(recordId, fieldName);
    });
  },
...
var _loop2 = function _loop2() {
  ...
  CollectionService.prototype[method] = function (value, context) {
    var iterator = _underscore["default"].isFunction(value) ? value : function (thisRecordId) {
      return _this7._recordsById[thisRecordId][value];
    };
    return _underscore["default"][method](_this7._recordIds, iterator, context);
  };
};
