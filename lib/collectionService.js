...
var CollectionService = module.exports = _baseService["default"].extend({
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              ...
              url = _this4._getRESTEndpoint(method, recordId);
              dto = _this4._recordToDTO(recordId, method);
              _context3.next = 7;
              return _this4._sync(url, isUpdate ? 'PUT' : 'POST', dto, options.ajax);

            case 7:
              result = _context3.sent;

              if (result.success) {
                if (!isUpdate) _this4._newRecordIds = _underscore["default"].without(_this4._newRecordIds, recordId);
                if (options.merge) _this4._mergeDTO(result.data);

                _this4.trigger('save', recordId, isUpdate, options);
              }
    ...
    return this[options.first ? 'find' : 'filter'](function (thisRecordId) {
      if (!options.ignoreMissingData) {
        for (var key in attrs) {
          if (_underscore["default"].isUndefined(_this5._recordsById[thisRecordId][key])) {
            throw new Error('Field \'' + key + '\' is not present for record id ' + thisRecordId + ' in table \'' + _this5.collectionName + '\'.');
          }
        }
      }

      return (0, _matchesWhereQuery["default"])(_this5._recordsById[thisRecordId], attrs);
    });
  },
  ...
  _getRESTEndpoint: function _getRESTEndpoint(method, recordIdOrIds, variableParts) {
    ...
    if (['update', 'delete', 'patch', 'get'].includes(method) && !_underscore["default"].isArray(recordIdOrIds)) {
...
var _loop = function _loop() {
  var thisMethodName = _underscoreTableMetho[_i];
...
for (var _i = 0, _underscoreTableMetho = underscoreTableMethodNames; _i < _underscoreTableMetho.length; _i++) {
  _loop();
...
var _loop2 = function _loop2() {
  var method = _attributeMethods[_i2];
  ...
  CollectionService.prototype[method] = function (value, context) {
    ...
    var iterator = _underscore["default"].isFunction(value) ? value : function (thisRecordId) {
      return _this6._recordsById[thisRecordId][value];
    };
    return _underscore["default"][method](this._recordIds, iterator, context);
  };
};

for (var _i2 = 0, _attributeMethods = attributeMethods; _i2 < _attributeMethods.length; _i2++) {
  _loop2();
}